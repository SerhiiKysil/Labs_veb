...
interface UpdateOrganizationFormProps {
    organizationId?: number | string | null; // Updated to be more flexible
  }

  const UpdateOrganizationForm: React.FC<UpdateOrganizationFormProps> = ({ organizationId }) => {
    ...
    useEffect(() => {
      const fetchOrganizationData = async () => {
        // Convert organizationId to string to handle both number and string formats
        ...
        try {
          const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/organizations/${orgIdString}`, {
            credentials: 'include',
          });
    ...
    }, [organizationId, userCurrent?.email]);
  ...
  const handleUpdateOrganization = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setSuccessMessage('Організацію оновлено успішно!');
        setTimeout(() => {
          router.push('/profile');
        }, 2000);
      } else {
        const errorData = await response.json();
        setErrorMessage(errorData.message || 'Не вдалося оновити організацію. Спробуйте ще раз.');
      }
    } catch (error) {
      console.error("Error:", error);
      setErrorMessage('Сталася помилка при оновленні організації. Спробуйте ще раз.');
    } finally {
    ...
    <section className="bg-white text-dark-blue py-12">
      <div className="container mx-auto grid grid-cols-1 gap-8" style={{ maxWidth: '85%' }}>
        ...
        <h2 className="text-2xl font-bold mb-6 text-center">
            {organizationId ? 'Оновити організацію' : 'Створити організацію'}
          </h2>
          ...
          >
            {isSubmitting ? 'Оновлення...' : (organizationId ? 'Оновити організацію' : 'Створити організацію')}
          </button>
